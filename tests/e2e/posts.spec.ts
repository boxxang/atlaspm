...
test.describe('key info', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.locator('.pcomposer')).toBeVisible();
  });
  ...
  test('a note is written, counted on the tab, and survives a reload', async ({ page }) => {
    ...
    await expect(page.locator('.ppost-text')).toHaveText('PDK 2.1 decks land in March.');
  ...
  test('a note is edited in place, and says it was edited', async ({ page }) => {
    ...
    await expect(page.locator('.ppost-text')).toHaveText('Second wording.');
    await expect(page.locator('.ppost-head')).toContainText('edited');
    await writesSettled(page);
  ...
  test('a note is deleted, and stays deleted', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Post' }).click();
    await expect(page.locator('.ppost')).toHaveCount(1);
    await page.getByRole('button', { name: 'Delete' }).click();
    await expect(page.locator('.ppost')).toHaveCount(0);
    await writesSettled(page);
  ...
  test('a reply sits under the post it answers, and goes with it', async ({ page }) => {
    ...
    await page.locator('.pcomposer.small').getByRole('button', { name: 'Post' }).click();

    await expect(page.locator('.preplies .ppost-text')).toHaveText('The answer.');
    await writesSettled(page);
    await page.reload();
    await expect(page.locator('.preplies .ppost-text')).toHaveText('The answer.');

    /* deleting the parent takes the thread — a reply to nothing is not a post */
    await page.locator('.pposts > li > .ppost').getByRole('button', { name: 'Delete' }).click();
    await expect(page.locator('.ppost')).toHaveCount(0);
    await writesSettled(page);
    await page.reload();
    await expect(page.locator('.ppost')).toHaveCount(0);
...
test.describe('posting on a step', () => {
  test('an update lands on that step and nowhere else', async ({ page }) => {
    ...
    await expect(rail(page).locator('.ppost-text')).toHaveText('Waiting on the spacing study.');
    await writesSettled(page);
  ...
  test('ticking risk makes it a risk, and the boards pick it up', async ({ page }) => {
    ...
    await expect(rail(page).locator('.ppost .ppill.risk')).toHaveText('Risk');
    await writesSettled(page);
    ...
    await expect(page.locator('.pboard tbody tr')).toHaveCount(7);
    await expect(page.locator('.pboard')).toContainText('Antenna fixes need another routing turn.');
  });
  ...
  test('a risk raised here closes when its step is handed over', async ({ page }) => {
    ...
    await expect(rail(page).locator('.ppost-text')).toHaveText('Blocked on the DRC deck.');
    await expect(rail(page).locator('.ppost .ppill.risk')).toHaveText('Risk');
  });
});
