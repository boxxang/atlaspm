...
import { getMeetingsState, getProjectState } from '@/lib/queries';
...
export default async function ProgramLayout({
  ...
  const [initial, meetings] = await Promise.all([
    getProjectState(projectId),
    getMeetingsState(projectId),
  ]);
  if (!initial) notFound();
  return (
    <ProgramShell initial={initial} meetings={meetings}>
      {children}
    </ProgramShell>
  );
}
